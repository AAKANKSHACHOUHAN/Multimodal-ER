...
df = pd.read_csv(r'', header=0, index_col=0)
...
y_less = df_less.iloc[0:10630, 62].values
# Extract features of majority classes.
X_less = df_less.iloc[0:10630, list(range(62))].values
print(y_less)

# Split and stratify majority class samples for training and testing.
X_train_temp_less, X_test_less, y_train_temp_less, y_test_less = train_test_split(X_less, y_less, test_size=1063, random_state=0, stratify=y_less) # training split = 90%, test split = 10%

# Further split and stratify majority class training samples for training data for training and validating.
X_train_less, X_val_less, y_train_less, y_val_less = train_test_split(X_train_temp_less, y_train_temp_less, test_size=1063, random_state=0, stratify=y_train_temp_less) # training split = 80%, validation split = 10%
...
plt.ylabel('Accuracy')
plt.title('SVM_Custom_Complete_Clean')
plt.legend(loc='lower right')
plt.ylim([0.0, 1.0])
#plt.show()

# Save plot as PNG file
fig.savefig('Learning_Curves_SVM_Librosa_Complete_Clean.png')
