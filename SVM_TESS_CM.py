...
df = pd.read_csv(r'/home/r_m727/All_Files/Corpus/Simulated/TESS/TESS_Librosa_Clean.csv', header=0, index_col=0)
...
y = df.iloc[0:2800, 62].values
# Extract features in a matrix X. Indexing from 0
X = df.iloc[0:2800, list(range(62))].values
print(y)
...
X_train_temp, X_test, y_train_temp, y_test = train_test_split(X, y, test_size=280, random_state=0, stratify=y) # training split = 90%, test split = 10%
...
X_train, X_val, y_train, y_val = train_test_split(X_train_temp, y_train_temp, test_size=280, random_state=0, stratify=y_train_temp) # training split = 80%, validation split = 10%
...
fig = plt.figure()
plot_confusion_matrix(confmat, class_names, title='SVM_Custom_TESS_Clean')
#plt.show()

# Save plot as PNG file
fig.savefig('Confusion_Matrix_SVM_Librosa_TESS_Clean.png')
